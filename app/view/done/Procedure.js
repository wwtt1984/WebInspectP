...
Ext.define('WebInspect.view.done.Procedure', {
    extend: 'Ext.Panel',
    xtype: 'procedure',
    ...
    config:{
        ...
        tpl:  Ext.create('Ext.XTemplate',
            '<div style="height:2em;width:100%;padding:0 5px 0 5px; font-size:15px;line-height:2em;background:#ddd;filter:alpha(opacity=50);-moz-opacity:0.5;opacity:0.5;">事件处理流程</div>',
...
//                    '<div style="width:83%;min-height:2.6em;float:left;border-top:1px #ccc solid;font-size:14px;line-height:2em;">',
                        '<p style="font-weight:bold;">{data.NodeName}</p>',
                        '<p>{data.displayname}({data.oulevel})</p>',
                        '<p>{data.ReceiveAt}</p>',
                    '</div>',
                    '<br style="clear:both;" />',
                '</div>',
            ...
            '</div>',
            {
                setTplContent: function(values){
                    ...
                    if(values.data.FinishAt){

                        str = '<div style="width:15%;height:100%;float:left;">' +
                            '<img src="resources/images/prepoint.png" style="width:11px;height:11px;margin:10px 0 0 15px;"/></div>' +
...
//                            '<div style="width:83%;min-height:2.6em;float:left;font-size:14px;border-top:1px #ccc solid;line-height:2em;">'+
//                            '<p style="font-weight:bold;">' + values.data.NodeName + '</p>'+
//                            '<p>' + values.data.displayname + '(' + values.data.oulevel + ')' + '</p>'+
//                            '<p>' + values.data.ReceiveAt + '</p>'+
//                            '</div>'+
//                            '<br style="clear:both;" />' +
//                            '</div>' ;
                    }
                        ...
                        str = '<div style="width:15%;height:100%;float:left;">' +
                            '<img src="resources/images/prefirst.png" style="width:13px;height:13px;margin:10px 0 0 14px;"/></div>' +
